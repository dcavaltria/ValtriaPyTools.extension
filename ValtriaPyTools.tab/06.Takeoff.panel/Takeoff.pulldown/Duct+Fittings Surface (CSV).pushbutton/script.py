...
# --- WPF robusto ------------------------------------------------------
try:
    clr.AddReference('WindowsBase')             # Necesario para WPF
    ...
    forms.alert(u"No se pudo cargar WPF (.NET):\n\n{}".format(ex), exitscript=True)

# Comprobación de engine
try:
    engine = "IronPython" if sys.version.lower().find("ironpython") >= 0 else "CPython"
    print("Engine:", engine, "| Python", sys.version)
except:
    pass

# --- XAML (asegúrate que sea unicode en IPy2) ------------------------
XAML = u"""
...
def _load_wpf():
    # Más tolerante que Load(XmlReader)
...
def calc_duct_rows(ducts, ut_global, ut_map):
    ...
    for d in ducts:
        try:
            ...
            if D > 0.0:
                shape = "Circular"; perim = perim_circ(D)
            else:
...
def summarize(rows, catlabel, col_system, col_level, col_phase, col_area):
    ...
    # orden: system, level, phase, category
